...
// MAGIC %md
// MAGIC ** Q-3) Can we compute min, max values for temperature, C02, and humidity?**
...
// MAGIC %md
// MAGIC ** Q-4) Can we sort and group country with average temperature, C02, and humidity?**

// COMMAND ----------

ds.filter(d => {d.temp > 25 && d.humidity > 75})
  .select("c02_level", "temp", "humidity", "cn")
  .groupBy($"cn")
  .avg()
  .sort($"avg(c02_level)".desc, $"avg(temp)".desc, $"avg(humidity)".desc).as("avg_humidity").show(10, false)